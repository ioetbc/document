...
export function createCalculationPlugin(getSourceValues: () => Record<string, number> = () => ({})) {
  return new Plugin<Set<number>>({
    ...
    appendTransaction(transactions, _oldState, state) {
      if (!transactions.some(transaction => transaction.docChanged || transaction.getMeta("initializeCalculations") || transaction.getMeta("sourceValuesChanged"))) return null;
      const markType = state.schema.marks.calculation_result;
      if (!markType) return null;
      const values = { ...getSourceValues(), ...extractSchema(state.doc.textBetween(0, state.doc.content.size, "\n", "\n")) };
...
export const calculationPlugin = createCalculationPlugin();
